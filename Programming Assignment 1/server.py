...
'''
import socket, re, select, sys

def mathValidator(data):
    #found pattern to match at https://stackoverflow.com/questions/34244782/check-whether-an-expression-is-valid-using-regex-in-python
    if re.match("^\s*[+-]?\s*(?:\d+(?:\.\d*)?|\.\d+)(?:\s*[-+/*]\s*\s*[+-]?\s*(?:\d+(?:\.\d*)?|\.\d+))*\s*$", data):
        return True
    else: return False
